...
/** Owner v3 reward-value correction. Frozen version IDs preserve old saved prizes.
 * Gold sectors weight 4; empty sectors and the three-spin sector weight 1. */
export const fortuneV2:readonly WheelSegment[]=[
 '25000','3000',null,'5000','10000','4000','free','6000',null,'15000','3500','8000',
].map((amount,index)=>({id:`fortune_v2_${index}`,index,weight:amount===null||amount==='free'?1:4,
 grants:amount===null?[]:amount==='free'?[{kind:'freeSpin',id:'fortune',amount:'3'}]:[{kind:'currency',id:'gold',amount}]}));
export function wheelSectors(outcomeId?:string):readonly WheelSegment[]{return outcomeId?.startsWith('fortune_v1_')?fortuneV1:fortuneV2;}
...
export function wheelSegment(outcomeId:string):WheelSegment {
  const segment=[...fortuneV1,...fortuneV2].find(s=>s.id===outcomeId);
...
export function selectWheelOutcome(spinId:string,rewardState:readonly number[],existing?:WheelSelection,segments:readonly WheelSegment[]=existing?wheelSectors(existing.outcomeId):fortuneV2):WheelSelection {

...
export function validateSave(value:unknown,supportedDataVersions?:readonly string[],allowLegacyCapacity=false):GameState {
 ...
 assertState(state,undefined,allowLegacyCapacity);new SeededRng(state.data.rng.combatState);new SeededRng(state.data.rng.rewardState);
...
export function decodeSave(bytes:string,supportedDataVersions?:readonly string[]):GameState {
 ...
 // Old all-slots-open saves still store 1/2; the loader upgrades them through
 // its durable repair transaction before publishing. New writes stay strict.
 return validateSave(JSON.parse(e.payload),supportedDataVersions,true);
}

...
import {deploymentCapacity as earnedDeploymentCapacity} from '../systems/AccountProgression';
...
export function assertState(state:GameState|Snapshot,deploymentCap=state.data.progression.deploymentCapacity??3,allowLegacyCapacity=false):void{
 assertEquipment(state);
 const {data}=state;
 // Missing capacity is accepted only for decoding legacy saves, then repaired
 // durably by the loader. New-format saves must agree with the earned level.
 const earned=earnedDeploymentCapacity(state),oldCapacity=data.player.accountLevel>=2?2:1;
 const legacy=allowLegacyCapacity&&earned===3&&data.progression.deploymentCapacity===oldCapacity;
 if(data.progression.deploymentCapacity!==undefined&&data.progression.deploymentCapacity!==earned&&!legacy)throw Error('Invalid unlocked deployment capacity');

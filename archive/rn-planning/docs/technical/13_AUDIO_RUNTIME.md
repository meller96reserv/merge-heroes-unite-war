...
0.2.3 ownership correction (TASK-0229): native one-shots and loops explicitly
pause/remove/release Expo players; removing a registry entry alone leaves native
decoders/AudioTracks until GC. Completion, stealing, disposal, partial startup
and error paths clean up exactly once. Bundled one-shots use their known duration
plus startup slack as a lost-callback deadline; no missed sound is replayed.
[Defect/retest](../../analysis/reports/qa_defect_register.md).

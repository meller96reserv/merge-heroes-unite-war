...
Dependencies: [TASK-0075](TASKS_PHASE_06.md), [TASK-0093](TASKS_PHASE_07.md), [TASK-0147](TASKS_PHASE_12.md)
...
Delivery classification: **REQUIRED_NOW**. Explicit owner request: bounded first-session buy/buy/merge/deploy/auto-combat tutorial, no broader reference tutorial.

[Delivery scope and gate](../plans/DELIVERY_SCOPE.md). Preserved internal/reference specifications do not expand this scope.

### Goal

Current owner override: Five persistent steps in existing Battle UI; real command/attack events only; forward reconciliation and a small Skip. No framework, separate scene, grants, advanced mechanics or exclusive input mask. The older broad reference sequence/input-mask requirements below are historical and do not expand this task.
...
<a id="task-0142"></a>
## TASK-0142 — Minimal first-session guidance

Phase: 11 · Priority: P1 · Type: implementation · Status: COMPLETED

Dependencies: [TASK-0141](TASKS_PHASE_11.md), [TASK-0063](TASKS_PHASE_05.md)

Parallelization: Allowed after listed dependencies; exclusive ownership of app/src/ui/TutorialOverlay.tsx. Integrate shared registries coherently.
...
Current owner override: Five persistent steps in existing Battle UI; real command/attack events only; forward reconciliation and a small Skip. No framework, separate scene, grants, advanced mechanics or exclusive input mask. The older broad reference sequence/input-mask requirements below are historical and do not expand this task.
Highlight the current target with short tooltip; keep other controls available and remove hints on route change/completion.

### Context
Bounded tutorial input mask deliverable. Authoritative output: `app/src/ui/TutorialOverlay.tsx`. Adjacent systems remain separate tasks.
...
### Files expected to create
- `app/src/ui/TutorialOverlay.tsx`
...
### Implementation steps
1. Read listed sources and verify dependency acceptance.
2. Highlight the current target with short tooltip; keep other controls available and remove hints on route change/completion.
3. Exercise the concrete edge cases and retain exact expected/actual evidence.
4. Update task/evidence traceability and commit only accepted coherent work.
...
### Manual verification
Exercise `app/src/ui/TutorialOverlay.tsx` through the smallest relevant RN/Skia/core/platform harness. Highlight the current target with short tooltip; keep other controls available and remove hints on route change/completion. Record platform limitations separately.

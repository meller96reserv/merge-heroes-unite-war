...
import {BOOST_OFFER_MS} from '../commands/ClaimStageBoost';
/** PROPOSED dragon-v2 pacing: supplied identities/rewards, shipped challenge HP. */
export const dragons=[
 {id:'infernal',name:'Infernal Dragon',hp:60000,gold:'500',gems:'50',orbs:'25',visualId:'boss_infernal_dragon__1968x1659',banner:'background_dungeon_infernal_strip__2172x724'},
 {id:'frost',name:'Frost Dragon',hp:200000,gold:'1000',gems:'75',orbs:'50',visualId:'boss_frost_dragon__1995x1925',banner:'background_dungeon_frost_strip__2172x724'},
 {id:'shadow',name:'Shadow Dragon',hp:600000,gold:'1500',gems:'100',orbs:'75',visualId:'boss_shadow_dragon__1974x1657',banner:'background_dungeon_shadow_bright_strip__2172x724'},
] as const;
...
export function finishDragon(state:GameState,c:FinishDragon,catalog:RewardCatalog,clock:Pick<Clock,'utcMs'>,stepMs:number):Reduction{
 ...
 const grants:Grant[]=c.outcome==='won'?[{kind:'currency',id:'gold',amount:definition.gold},{kind:'currency',id:'gem',amount:definition.gems},{kind:'currency',id:'orb',amount:definition.orbs}]:[];
 ...
 if(c.outcome==='won'){
  ...
  records[id]={id,encounterId:active.id,stageId:`dungeon_${definition.id}`,baseGold:definition.gold,createdAt:now,offerEndsAt:now+BOOST_OFFER_MS,status:'available',multiplier:null};state.data.stageBoosts=records;
  return {ok:true,events:[...events,{type:'stageBoost.available',payload:{boostId:id,baseGold:definition.gold,offerEndsAt:now+BOOST_OFFER_MS}}]};
 }

...
export function registerStageBoost(state:GameState,clear:{encounterId:string;stageId:string;commandId:string},clock:Pick<Clock,'utcMs'>):Intent[] {
 ...
 const boost:StageBoost={id,encounterId:clear.encounterId,stageId:clear.stageId,baseGold:base.toString(),createdAt:now,offerEndsAt:now+BOOST_OFFER_MS,status:'available',multiplier:null};
...
export function reserveStageBoost(state:GameState,command:ReserveStageBoost,clock:Pick<Clock,'utcMs'>):Reduction {
 ...
 if(boost.status==='reserved')return {ok:true,events:[]};
...
export function claimStageBoost(state:GameState,command:ClaimStageBoost,catalog:RewardCatalog):Reduction {
 ...
 const total=BigInt(boost.baseGold)*BigInt(boost.multiplier),bonus=total-BigInt(boost.baseGold),grants=[{kind:'currency' as const,id:'gold',amount:bonus.toString()}];

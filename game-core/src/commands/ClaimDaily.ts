...
import {dailyEligibility,DAILY_GOLD} from '../systems/DailyService';
...
export function reserveDaily(state:GameState,c:ReserveDaily,clock:Pick<Clock,'utcMs'>):Reduction{
 ...
 state.data.daily.pendingPeriod=eligibility.day;metaReceipt(state,c.commandId,source);return {ok:true,events:[{type:'daily.reserved',payload:{period:eligibility.day}}]};
}
export function claimDaily(state:GameState,c:ClaimDaily,catalog:RewardCatalog):Reduction{
 ...
 const {receipt}=grantReward(state,c.commandId,source,[{kind:'currency',id:'gold',amount:DAILY_GOLD}],catalog);
 state.data.daily.pendingPeriod=null;state.data.daily.lastClaimedPeriod=c.period;state.data.daily.attendanceIndex++;

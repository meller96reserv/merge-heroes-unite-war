...
export function deployHero(state:GameState,command:DeployHero,cap:number):Reduction {
 ...
 if(!Number.isInteger(cap)||cap<1||cap>15)return {ok:false,reason:'INVALID_COMMAND'};
 ...
 if(command.deployed&&state.data.heroes.filter(h=>h.deployed).length>=cap)return {ok:false,reason:'LOCKED'};
